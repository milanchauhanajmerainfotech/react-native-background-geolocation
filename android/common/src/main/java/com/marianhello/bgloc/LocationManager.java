...
public class LocationManager {
    ...
    public static final String[] PERMISSIONSOVER = {
            Manifest.permission.ACCESS_COARSE_LOCATION,
            Manifest.permission.ACCESS_FINE_LOCATION,
    ...
    public Promise<Location> getCurrentLocation(final int timeout, final long maximumAge, final boolean enableHighAccuracy) {
        Promise<Location> promise = Promises.promise();

        PermissionManager permissionManager = PermissionManager.getInstance(mContext);
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            promise = checkPermission(permissionManager, PERMISSIONSOVER, timeout, maximumAge, enableHighAccuracy);
        } else {
            promise = checkPermission(permissionManager, PERMISSIONS, timeout, maximumAge, enableHighAccuracy);
        }
        return promise;
    }

    public Promise<Location> checkPermission(PermissionManager permissionManager, String[] PERMISSIONSTYPE, final int timeout, final long maximumAge, final boolean enableHighAccuracy) {
        final Promise<Location> promise = Promises.promise();
        permissionManager.checkPermissions(Arrays.asList(PERMISSIONSTYPE), new PermissionManager.PermissionRequestListener() {
            @Override
